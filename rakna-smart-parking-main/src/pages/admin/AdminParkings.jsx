...
export default function AdminParkings() {
  ...
  return (
    <AdminLayout>
      <h1 style={{ fontSize: '1.4rem', fontWeight: 800, color: C.black, margin: '8px 0 16px', fontFamily: FONT }}>{t('a_parkings')}</h1>
        ...
        {lots.map((lot) => {
          ...
          return (
            <div key={lot.id} style={{ background: C.white, borderRadius: R.card, padding: 18, boxShadow: SHADOW.soft }}>
              <div style={{ fontWeight: 700, color: C.black, fontFamily: FONT }}>{lot.name}</div>
              <div style={{ color: C.textMuted, fontSize: '0.78rem', marginBottom: 12, fontFamily: FONT }}>📍 {lot.address}</div>
              <div style={{ background: C.grey, borderRadius: R.sm, height: 8, marginBottom: 14 }}>
                <div style={{ width: pct + '%', height: '100%', borderRadius: R.sm, background: pct > 80 ? C.danger : pct > 50 ? C.reserved : C.available, transition: 'width 0.3s' }} />
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
                <Num label={t('available_spots')} value={lot.availableSpots} onChange={(v) => setField(lot.id, 'availableSpots', v)} />
                <Num label="الإجمالي"             value={lot.totalSpots}     onChange={(v) => setField(lot.id, 'totalSpots', v)} />
              ...
              <button onClick={() => save(lot)} disabled={savingId === lot.id} style={{
                ...
                background: savedId === lot.id ? 'var(--success)' : 'var(--brand)',
                color: 'var(--on-ink)',
                fontWeight: 700, fontSize: '0.85rem', cursor: 'pointer', fontFamily: FONT,
              }}>
...
function Num({ label, value, onChange }) {
  return (
    <div>
      <div style={{ fontSize: '0.68rem', color: C.textMuted, marginBottom: 4, fontFamily: FONT }}>{label}</div>
      <input type="number" value={value} onChange={(e) => onChange(e.target.value)} style={{
        width: '100%', padding: '9px 8px', border: '1.5px solid var(--border)', borderRadius: R.sm,
        fontSize: '0.9rem', outline: 'none', boxSizing: 'border-box',
        background: C.white, color: C.black, textAlign: 'center', fontFamily: FONT,
      }} />
    </div>
  )
}