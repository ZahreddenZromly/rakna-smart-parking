...
export default function AdminDashboard() {
  ...
  const cards = [
    ...
    { label: t('online_now'),     value: stats.online,           icon: 'user',   color: '#2F8F5B', to: '/admin/users' },
    { label: t('total_bookings'), value: stats.bookings,         icon: 'ticket', color: '#A55EEA', to: '/admin/analytics' },
    { label: t('total_revenue'),  value: stats.revenue + ' د.ل', icon: 'wallet', color: '#3B82F6', to: '/admin/analytics' },
  ]
  ...
  return (
    <AdminLayout>
      <h1 style={{ fontSize: '1.5rem', fontWeight: 800, color: C.black, margin: '8px 0 18px', fontFamily: FONT }}>{t('admin_dashboard')}</h1>
      ...
      <div className="stagger" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px,1fr))', gap: 14 }}>
        {cards.map((c) => (
          <div key={c.label} onClick={() => navigate(c.to)} style={{
            ...
            boxShadow: SHADOW.soft, borderLeft: '5px solid ' + c.color, cursor: 'pointer',
          }}>
            <Icon name={c.icon} size={24} color={c.color} />
            <div style={{ fontSize: '1.7rem', fontWeight: 800, color: C.black, marginTop: 6, fontFamily: FONT }}>{c.value}</div>
            <div style={{ color: C.textMuted, fontSize: '0.85rem', fontFamily: FONT }}>{c.label}</div>
          </div>
        ))}
      </div>

      <h2 style={{ fontSize: '1.05rem', fontWeight: 700, color: C.black, margin: '26px 0 12px', fontFamily: FONT }}>{t('admin_panel')}</h2>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px,1fr))', gap: 14 }}>
        {quick.map((q) => (
          <button key={q.label} onClick={() => navigate(q.to)} style={{
            background: C.white, borderRadius: R.card, padding: '22px 16px',
            boxShadow: SHADOW.soft, border: 'none', outline: 'none',
            cursor: 'pointer', textAlign: 'center', fontFamily: FONT,
          }}>
            <div style={{ display: 'flex', justifyContent: 'center' }}>
              <Icon name={q.icon} size={28} color="var(--brand)" />
            </div>
            <div style={{ fontWeight: 700, color: C.black, marginTop: 8, fontFamily: FONT }}>{q.label}</div>
          </button>
        ))}
      </div>
    </AdminLayout>
